...
select pg_temp.zero((select count(*) from (select scope_type, role_key, capability_key from public.role_capability_defaults_legacy
  except select scope_type, role_key, capability_key from public.role_capability_defaults) x) - 44, 'F8: legacy role defaults lost only the 34 intended removals (bundle_legacy_parity names them)');
