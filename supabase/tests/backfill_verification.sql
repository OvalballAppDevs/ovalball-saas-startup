...
select pg_temp.zero((select count(*) from (select scope_type, role_key, capability_key from public.role_capability_defaults_legacy
  except select scope_type, role_key, capability_key from public.role_capability_defaults) x) - 27, 'F8: legacy role defaults lost only the 27 intended removals (bundle_legacy_parity names them)');
