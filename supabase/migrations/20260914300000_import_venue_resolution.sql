-- Venue instruction Section 20: CSV import must resolve venue against the
-- importing club's real venues, never silently invent one from a typo --
-- mirrors resolved_pitch_id/resolved_competition_edition_id exactly.
-- fixture_import_rows.resolved_venue_id already existed on the ORIGINAL
-- base table (20260831250000) -- declared from the start but never
-- populated by anything, since no CSV column reader/resolver ever wrote
-- to it until now. No column to add here, just wire it up.

comment on column public.fixture_import_rows.resolved_venue_id is
  'Real venues.id resolved from the CSV''s venue_id/venue_name columns, or a staging-review correction -- never created from CSV text. Carried through to the created/updated fixture by publish_import_row, same as resolved_pitch_id.';
...
comment on function public.publish_import_row(uuid) is
  'The only path that turns a staged fixture_import_rows row into a real fixtures row. Atomic per row. resolved_status/resolved_home_score/resolved_away_score carry through when a historical-backfill row set them -- a populated score pair always sets result_status = ''external_recorded''. resolved_venue_id carries through the same way resolved_pitch_id already does.';
