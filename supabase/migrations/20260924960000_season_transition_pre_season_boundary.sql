-- RESUME SEASON HANDOVER, Section 1: the automatic team/cohort identity
-- transition boundary is the START OF THE NEW SEASON CYCLE
-- (to_season.pre_season_starts_on), never main-season start
-- (to_season.starts_on) -- the Canonical Seasons audit correctly
-- disclosed the previous engine used starts_on. A U13 cohort becomes the
-- new season's U14 cohort the day pre-season opens (e.g. 1 June),
-- regardless of when the competition itself begins (e.g. 1 September).
--
-- Never hardcoded: the boundary is read entirely from the canonical
-- target season row, for whichever rugby_code is actually being
-- processed. If a season has no configured pre_season_starts_on, this
-- is NOT silently treated as "transition at main season start instead"
-- -- that would resurrect exactly the bug this fix closes. Instead the
-- transition is explicitly parked at needs_attention with a stated
-- reason the moment it would otherwise become due, so a human decides
-- rather than the system guessing.
alter table public.season_transitions add column needs_attention_reason text;
comment on column public.season_transitions.needs_attention_reason is
  'Human-readable reason a transition is parked at needs_attention -- either "N team(s) still need a manual decision" (the original, mechanical-progression case) or a structural problem the engine refuses to guess past, e.g. "No pre-season start date is configured for <season>."';
