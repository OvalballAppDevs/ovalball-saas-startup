-- =====================================================================
-- THE INVITATION BELONGS IN TOURNAMENT CENTRE
--
-- Responding to a tournament invitation lived in a Calendar pop-over, which
-- was the only place a tournament had any surface at all. Now that a
-- tournament has its own Centre, the Calendar entry opens that Centre -- and
-- the one thing the pop-over uniquely did has to travel with it, or the flow
-- would simply be gone.
--
-- This adds `invitations` to the Tournament Centre payload: the participant
-- rows at this tournament that the VIEWER is entitled to answer for, and
-- nothing else. Answering still goes through the existing
-- public.respond_tournament_invitation, which is unchanged -- this migration
-- adds a read, not a second way to accept.
-- =====================================================================
...
begin
  ...
  select jsonb_build_object(
    ...
    -- INVITATIONS THIS VIEWER MAY ANSWER. Scoped by the same team/club
    -- authority respond_tournament_invitation itself checks, so the panel can
    -- never offer a button the RPC would refuse.
