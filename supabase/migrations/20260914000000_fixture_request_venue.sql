-- Venue integration gap (Lookup Administration follow-up): the request-
-- based fixture creation path (Club Admin/Fixtures Secretary, via
-- createFixtureRequest) never explicitly wrote fixtures.venue_id -- venue
-- only ever displayed correctly on read because the selected pitch's own
-- venue_id happened to resolve it. Mirrors fixture_requests.pitch_id's
-- exact existing pattern (20260907000000): proposed by the requesting
-- team, only meaningful when that team's venue_preference resolves to
-- Home in the final fixture, carried onto the resulting fixtures row by
-- accept_fixture_request only in that case.

alter table public.fixture_requests
  add column venue_id uuid references public.venues(id);

comment on column public.fixture_requests.venue_id is
  'Proposed by the requesting team, only when their venue_preference is Home -- carried onto the resulting fixtures row by accept_fixture_request when that side does end up Home. Never implies anything about the opponent''s venue.';

-- accept_fixture_request: identical to 20260907000000's definition, with
-- venue_id now flowing from the request onto the resulting fixtures row
-- alongside pitch_id, using the exact same "only when requesting side
-- ends up Home" rule.
