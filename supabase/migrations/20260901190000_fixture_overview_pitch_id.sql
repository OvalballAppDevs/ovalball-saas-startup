-- admin_fixture_overview never exposed the new fixtures.pitch_id (added by
-- club_pitches migration) -- the admin fixture detail page needs it to
-- resolve the current pitch selection the same way the messages page
-- already does. Appended at the end, matching every prior CREATE OR
-- REPLACE VIEW column-order comment in this view's history.
