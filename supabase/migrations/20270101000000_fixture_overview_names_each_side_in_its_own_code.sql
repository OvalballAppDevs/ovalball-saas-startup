-- Fixture Management names each side in its own rugby code.
--
-- The view already carried the structured fields the app runs through
-- fullTeamLabel -- category, age group, gender, squad designation -- but not
-- the rugby code, which is equally part of the name: Rugby League runs Open
-- Age where Rugby Union numbers its senior sides. Without it the fixture
-- table called a league club's senior team "Men's 1st Team", a name that club
-- does not use and would not recognise, while Site Admin's Team Directory and
-- the club's own Teams page called the same side "Men's Open Age".
--
-- Purely additive: the two new columns are appended, every existing column and
-- the whole join tree are untouched.
...
select f.id,
    ...
    -- What each side is CALLED depends on its own code: a league club's
    -- senior side is Men's Open Age, a union club's is Men's 1st Team.
    -- Side-swapped exactly like every other home_/away_ column, so a
    -- fixture between codes names each side the way its own club does.
    case when f.home_away = 'Away' then opp_t.rugby_code else t.rugby_code end as home_team_rugby_code,
    case when f.home_away = 'Away' then t.rugby_code else opp_t.rugby_code end as away_team_rugby_code
   from fixtures f
...
comment on view public.admin_fixture_overview is
  'Fixture Management''s (Site Admin and every Club Admin) single canonical fixture read surface. home_team_rugby_code/away_team_rugby_code complete the structured identity each side is named from, so a Rugby League side is never rendered with a Rugby Union senior name.';
