...
**Resolved in the closure pass**: the original disclosure here was that only this page computed `canSafeguarding` for `<ClubSettingsNav>`, so the tab was invisible from the other eight Club Settings surfaces. `app/(app)/club/settings/resolve-nav-capabilities.ts` is now the single canonical computation of every `<ClubSettingsNav>` boolean — one `Promise.all` of `hasCapability()` calls, called by all eleven consumer pages instead of each maintaining its own frozen copy of the list. Adding a future tab now means adding one field to this resolver, not editing eleven pages, which is the actual fix for the failure class (not just this one instance of it): the same audit also found `/club/player-moves` deriving `canTeams` from `canEditProfile || canVenues` (should be `canPitches`) and `/club/settings/pitch-allocation` hardcoding `canProfile`/`canTeams`/`canVenues`/`canRollover` to `false` regardless of the viewer's real capabilities — both silently wrong in the same way the safeguarding tab was, both now correct because they run through the shared resolver. `canPlayerMoves` itself was also inconsistent across pages (`manage_fixture_callups` alone on most call sites vs. `manage_fixture_callups OR manage_player_dispensations` on the Player Moves page's own gate); the resolver unifies on the `OR`, matching the page's own gate exactly.

## Closure pass addendum

A follow-up hardening pass (spec: "VERIFICATION & HARDENING CLOSURE PASS") found and fixed two further defects beyond the navigation propagation gap, both already covered by regression:

- **Client-supplied email/club-name trust**: `inviteSafeguardingOfficer`, `resendSafeguardingOfficerInvitation` and `messageSafeguardingOfficer` originally accepted `clubName`/`contactEmail`/`senderName` as plain arguments — values that had only ever been *rendered* server-side, but a Next.js server action is a real HTTP endpoint once compiled, so a tampered direct call could still supply arbitrary values. All three now resolve club name, recipient email and sender name from the officer assignment row and the authenticated session only, taking the officer/conversation id as the sole client input.
- **Open email relay via silent fallback**: `messageSafeguardingOfficer` treated *any* error from `start_or_get_safeguarding_officer_conversation` — including `42501` ("not authorized to message this club's officer") — as the signal to fall back to email, meaning an unauthorized caller's rejection was silently converted into a real email send under their own message text. Only the specific `22023` ("no active registered officer") error now triggers the fallback; every other error is returned as a refusal.

`supabase/tests/safeguarding_officer_security.sql` (31 assertions, transactional) is the permanent regression for this class of defect: cross-club nomination/invitation/conversation attempts, token replay/random-token/cross-club-token rejection, pending-grants-no-authority, acceptance-binds-existing-person-creates-no-duplicate, deactivation-preserves-person, capability isolation across clubs, and an explicit assertion (30/31) proving dispensation notifications remain gated on an actual capability grant rather than implied by acceptance.
