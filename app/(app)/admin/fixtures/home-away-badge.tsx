const STYLES: Record<string, string> = {
  ...
  "Not Applicable": "bg-ink/8 text-ink-muted",
}
...
export function HomeAwayBadge({ value }: { value: string }) {
  return (
    <span className={`rounded-full px-2.5 py-1 text-xs font-bold tracking-[0.04em] uppercase ${STYLES[value] ?? "bg-ink/8 text-ink-muted"}`}>
