...
export function TeamsPanel({ teams }: { teams: ClubTeamSummary[] }) {
  if (teams.length === 0) {
    return <p className="text-sm text-ink-muted">No teams recorded for this club yet.</p>
  }
      ...
      {teams.map((team) => (
        <li key={team.id} className="flex items-center justify-between gap-3 rounded-lg border border-ink/10 bg-white p-3.5">
          ...
          <div className="flex shrink-0 items-center gap-2">
            {!team.active && <span className="rounded-full bg-ink/8 px-2.5 py-1 text-xs font-medium text-ink-muted">Inactive</span>}
