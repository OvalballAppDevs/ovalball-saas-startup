...
import {
  correctClubImportRow,
  excludeClubImportRow,
  getMyClubPitchesForImport,
  getMyClubTeamsForImport,
  publishClubBatch,
  requireClubImportAccess,
  resolveClubRowConflict,
} from "../actions"
...
export default async function ClubImportBatchPage({ params }: { params: Promise<{ batchId: string }> }) {
  ...
  // club_id mismatch is treated the same as not-found -- a club must
  // never even learn that a batch id belonging to a different club (or a
  // Site Admin global import) exists, let alone see its rows. RLS already
  // blocks the underlying row reads; this makes the page behaviour match.
  if (!batch || batch.club_id !== clubId) notFound()

  const { data: rows } = await supabase.from("fixture_import_rows").select("*").eq("batch_id", batchId).order("row_number")
  const clubTeams = await getMyClubTeamsForImport()
  const clubPitches = await getMyClubPitchesForImport()
  ...
  const { data: conflictingFixtures } =
    conflictingIds.length > 0
      ? await supabase
          .from("admin_fixture_overview")
          .select("id, owning_team_name, opponent_team_name, raw_opposition_text, kickoff_date, kickoff_time, game_type, status")
    ...
    <div className="mx-auto max-w-4xl px-4 py-8 md:px-8 md:py-12">
      ...
      <div className="mt-6">
        <ImportReviewPanel
          ...
          resolveRowConflict={resolveClubRowConflict}
          excludeImportRow={excludeClubImportRow}
          publishBatch={publishClubBatch}
          correctRow={correctClubImportRow}
          clubTeams={clubTeams}
